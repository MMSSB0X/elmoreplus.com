...
// Global Cache so we don't spam Firestore reads for the same user
const userCache = {};
...
function startFeedListener() {
    ...
    onSnapshot(q, async (snapshot) => {
        // Step 1: Find all user UIDs in the feed that we haven't cached yet
        ...
        // Step 2: Fetch missing users asynchronously so the feed is completely up-to-date
        for (const uid of missingUids) {
            try {
                ...
                if (uDoc.exists()) {
                    ...
                    // Fallback if user deleted account
        ...
        // Step 3: Now that we have all live user data, render the feed
        feedContainer.innerHTML = "";
        ...
        snapshot.forEach((postDoc) => {
            ...
            // GET LIVE AUTHOR DATA FROM CACHE! 
            const liveAuthor = userCache[post.authorUid];
            
            // Override static post details with live profile details
...
async function createPost(text, btn) {
    ...
    try {
        await addDoc(collection(db, "posts"), {
            text: text,
            // Post will still save static details, but UI will now pull live details on render!
            author: currentUser.displayName, 
            authorPic: currentUser.photoURL,
            authorUid: currentUser.uid,
            createdAt: serverTimestamp(),
